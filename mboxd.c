...
static int dispatch_mbox(struct mbox_context *context)
{
	...
	switch (req.msg.command) {
		...
		case MBOX_C_GET_MBOX_INFO:
			...
			r = ioctl(-context->fds[LPC_CTRL_FD].fd, LPC_CTRL_IOCTL_MAP, &map);
			if (r < 0) {
				r = -errno;
				resp.msg.response = MBOX_R_SYSTEM_ERROR;
...
int main(int argc, char *argv[])
{
	...
	if (ioctl(context->fds[LPC_CTRL_FD].fd, LPC_CTRL_IOCTL_SIZE, &context->size) < 0) {
		r = -errno;
		MSG_OUT("fail\n");
